...
// Filter appointments if filters are applied
...
if (isset($_GET['doctor']) && !empty($_GET['doctor'])) {
    $filteredAppointments = array_filter($filteredAppointments, function($app) {
        return $app->getDoctorName() === $_GET['doctor'];
    });
}
            ...
            <div class="card-body">
                <form method="GET" action="">
                    <div class="row g-3">
                        ...
                        <div class="col-md-3">
                            <select class="form-select" name="doctor" id="doctorFilter" onchange="filterTable()">
                                <option value="">All Doctors</option>
                                <?php foreach ($doctors as $doc): ?>
                                    <option value="<?= htmlspecialchars($doc->getFullName()) ?>" <?= (isset($_GET['doctor']) && $_GET['doctor'] === $doc->getFullName()) ? 'selected' : '' ?>>
                            ...
                            <?php else: ?>
                                <?php foreach ($filteredAppointments as $app): ?>
                                    <tr>
                                        ...
                                        <td><?= htmlspecialchars($app->getDoctorName() ?? 'N/A') ?></td>
                                        <td>
                                            <div>
                ...
                <nav aria-label="Page navigation" class="mt-3">
                    <ul class="pagination justify-content-end">
                        ...
                        <li class="page-item active">
                            <a class="page-link" href="#" style="background-color: #3F0D12; border-color: #3F0D12;">1</a>
                        </li>
                        <li class="page-item">
                            <a class="page-link" href="#" style="color: #3F0D12;">Next</a>
                        </li>
                    </ul>
                </nav>
            </div>
                            ...
                            <select class="form-select" name="doctor_id" id="doctorSelect" required>
                                ...
                                <?php foreach ($doctors as $doctor): ?>
                                    <option value="<?= $doctor->getDoctorId() ?>">
                                        <?= htmlspecialchars($doctor->getFullName()) ?> - <?= htmlspecialchars($doctor->getSpecialization()) ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                        </div>
